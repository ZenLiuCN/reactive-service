...
/**
 * not ready to use. non tested
 * @author Zen.Liu
 * @param <T>
 */
public interface SingletonHolder<T> {
    /**
    ...
    static <T> SingletonHolder<T> generate(@Nullable final Supplier<T> instanceSupplier,
                                           @Nullable final Class<? extends T> spi) {
        return new Singleton<>(instanceSupplier, spi);
    }

    final class Singleton<T> implements SingletonHolder<T> {
        private SoftReference<T> softReference;
        private T hardReference;
        private final Supplier<T> instance;
        private final Class<? extends T> serviceProvider;

        Singleton(final Supplier<T> instance, final Class<? extends T> serviceProvider) {
